...
main_dir='/home/lmathon/Comparaison_pipelines/01_In_silico/06_chimera/Outputs/01_vsearch/main'
fin_dir='/home/lmathon/Comparaison_pipelines/01_In_silico/06_chimera/Outputs/01_vsearch/final'
...
nochim_sequence_sample="${clean_sequence_sample/.fasta/.nochim.fasta}"
echo "/usr/bin/time $vsearch --uchime3_denovo "$clean_sequence_sample" --sizeout --mindiffs 3 --mindiv 0.8 --relabel_keep --notrunclabels --nonchimeras "$nochim_sequence_sample >> $sample_sh
done
...
cat $main_dir/"$pref"_sample_*.uniq.l20.r005.clean.nochim.fasta > $all_sample_sequences_clean
