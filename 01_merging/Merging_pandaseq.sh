...
pandaseq='singularity exec /home/lmathon/ednatools.img pandaseq'
...
main_dir='/home/lmathon/Comparaison_pipelines/01_In_silico/01_Merging/Outputs/06_pandaseq/main'
fin_dir='/home/lmathon/Comparaison_pipelines/01_In_silico/01_Merging/Outputs/06_pandaseq/final'

######################################################################################################

# Assemblage des reads forward et reverse
/usr/bin/time $pandaseq -f $R1_fastq -r $R2_fastq -B -F -A rdp_mle -o 10 -w $main_dir/"$pref".fastq

######################################################################################################

## Assignation de chaque séquence à son échantillon
#$ngsfilter -t $sample_description_file -u $main_dir/"$pref"_unidentified.fastq $main_dir/"$pref".fastq --fasta-output > $main_dir/"$pref".ali.assigned.fasta
