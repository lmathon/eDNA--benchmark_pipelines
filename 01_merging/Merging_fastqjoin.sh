...
fastqjoin='singularity exec ednatools.img fastq-join'
...
/usr/bin/time $ngsfilter -t $sample_description_file -u $main_dir/"$pref"_unidentified.fastq $main_dir/"$pref"_join.fastq.gz --fasta-output > $main_dir/"$pref".ali.assigned.fasta
