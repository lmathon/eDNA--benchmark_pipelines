...
casper=${SINGULARITY_EXEC_CMD}" "${EDNATOOLS_SIMG}" casper"
...
main_dir=`pwd`"/benchmark_real_dataset2/01_merging/Outputs/04_casper/main"
fin_dir=`pwd`"/benchmark_real_dataset2/01_merging/Outputs/04_casper/final"


###################################################################################################################

## forward and reverse reads assembly
/usr/bin/time $casper $R1_fastq $R2_fastq -t 16 -w 10 -g 0.25 -j -o $main_dir/"$pref"
