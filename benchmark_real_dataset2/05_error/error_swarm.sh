...
main_dir=`pwd`"/benchmark_real_dataset2/05_error/Outputs/01_swarm/main"
fin_dir=`pwd`"/benchmark_real_dataset2/05_error/Outputs/01_swarm/final"
...
do
  ...
  echo "$obiannotate -S 'size:count' "$good_sequence_sample" | $container_python2 benchmark_real_dataset2/05_error/formate_header.py > "$formated_sequence_sample >> $sample_sh
